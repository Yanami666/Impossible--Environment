...
public class FPMovementController : MonoBehaviour
{
    ...
    public void Move()
    {
        if (!IsRunning)
        {
            rb.linearVelocity = moveDirection * walkSpeed * Time.fixedDeltaTime * 100;
        }
        else
        {
            rb.linearVelocity = moveDirection * runSpeed * Time.fixedDeltaTime * 100;
        }
    ...
    {
        if (IsGrounded() && rb.linearVelocity.magnitude > 5.0f && !audioSource.isPlaying)
        {
            if (IsRunning)
            {
